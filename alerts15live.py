...
cds = pd.read_sql('select * from contracts where sectype in ("IND", "STK")', con=db.engine)
...
class MyStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.skyler: Skyler = Skyler()
        self.skylerq: Optional[Queue] = None
        self.skylerthread = Thread(target=self.sendaskyler, name="skyler", daemon=True)
        self.skylerthread.start()
        self.skylerq.put("Market Starts")
    ...
    def notify_order(self, order):
        ...
        if xone.orders == [None]:  # All child orders processed
            ...
            self.skylerq.put(xone.notification())
                ...
                if xone.isbullish:
                    ...
                    if data.low[0] <= xone.entry:
                        if xone.status == XoneStatus.CREATED:
                            ...
                            self.skylerq.put(xone.notification())
                            row, index = 0, 0  # self.nextrowindex()
    ...
    def sendaskyler(self):
        self.skylerq = Queue()
        while True:
            # for chatid in [975198388, 1187821408, 156336062]:
            #     self.skyler.send_message(chatid, self.skylerq.get())
            self.skyler.send_all_clients(self.skylerq.get())
